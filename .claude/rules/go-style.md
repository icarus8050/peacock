...
- 경계(유저 입력, 외부 API) 바깥의 내부 코드에서는 발생 불가능한 경우에 대한 방어적 검증/분기를 추가하지 않는다.
  - **예외 — 외부 스트림 경계의 종결 메서드**: 종결 메서드(`Commit`/`Cancel`/`Close`)가 네트워크 스트림 수신 등 fault-prone한 외부 경계에서 소비되면, 잘못된 재호출(`Cancel` 후 `Commit`, 이중 `Commit`)은 "발생 불가능"이 아니다 — 중간 실패→정리→재시도가 현실적 흐름이다. 이때는 닫힌 fd 접근으로 panic하게 두지 말고 `done` 같은 종결 플래그 + sentinel error(`ErrXxxDone`)로 막는다. panic은 데이터 손상보다 진단이 어렵다.
...
- **doc가 약속한 부수효과는 본문과 대조한다.** "메타·데이터·tmp를 지운다" 같이 doc가 동작 목록을 나열하면, 본문이 실제로 그 전부를 수행하는지 한 번 훑는다. doc가 코드보다 많이 약속하면(코드는 메타·데이터만 지움) 다음 작업자가 거짓 보증을 믿는다 — doc를 줄이거나 본문을 맞춘다.
...
- **default-off 옵션은 entrypoint까지 배선됐는지 확인한다.** 새 Config/Options 필드가 기본값 off(0/false/"")로 동작을 끄면, main·상위 Options·CLI 플래그까지 노출돼 실제로 켤 수 있는지 본다. 라이브러리 레벨에만 있고 바이너리에서 못 켜면 기능 전체가 dead path가 되고, 라이브러리 단위 테스트는 직접 필드를 세팅해 통과하므로 거짓 안심을 준다. 자가 점검(변경 파일 한정)이 놓치기 쉬운 end-to-end 도달성 항목.
